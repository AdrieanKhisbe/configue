...
Configue._options = {};
['files', 'defaults', 'disable', 'env', 'argv', 'customWorkflow', 'required', 'overrides'].forEach((option) => {
    Configue[option] = (opt) => {
...
internals.Configue.prototype.getFirst = function () {
    for(let key of Array.from(arguments)) {
        const result = this.nconf.get(key);
        if (result !== undefined) return result;
    }
};

internals.Configue.prototype.getAll = function () {
    return Array.from(arguments).map((key) => this.nconf.get(key));
};
