...
exports.register = function register(server, options, next) {
    ...
    if (settings.customWorkflow) {
        settings.customWorkflow(nconf);
    } else {
       internals.applyDefaultWorkflow(nconf, settings);
    }
 ...
 */
internals.applyDefaultWorkflow = function (nconf, settings) {
...
internals.processHook = function processHook(hook) {
    if (!Array.isArray(hook) && typeof hook === 'function')
        hook(internals.nconf);
    else
        hook.forEach((func) => func(internals.nconf));
};
