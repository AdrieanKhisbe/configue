...
internals.Configue = module.exports = function Configue (options) {
    ...
    let settings = this.settings = Hoek.cloneWithShallow(options, 'provider');
    // settings validation
...
internals.Configue.prototype.resolve = function (callback) {
    ...
    if (this.settings.customWorkflow)
        this.settings.customWorkflow(this.nconf, callback);
    else internals.applyDefaultWorkflow(this.nconf, this.settings, callback);
    // TODO: resolve marqueur!!
}


internals.Configue.prototype.get = function get(key) {
 ...
 */
const hapiPlugin = internals.Configue.prototype.register = function register(server, options, next) {
    server.log(['plugin', 'info'], "Registering the hapi-configue plugin");

    const decorate = internals.decorate(server, this.nconf, next); // FIXME: this
 };
 ...
 * @param nconf - nconf object
 * @param next - end of plugin process callback
 * @returns {Function}
 */
