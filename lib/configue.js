...
exports.register = function register(server, options, next) {
    ...
    if (settings.customWorkflow)
        settings.customWorkflow(nconf, decorate);
    else internals.applyDefaultWorkflow(nconf, settings, decorate);
};
...
internals.loadDefaults = function loadDefaults(options) {
    ...
    if (Array.isArray(defaults) && defaults.length) {
        defaults.forEach(function (def, i) {
            internals.nconf.defaults('default' + i, def);
        })
    } else {
