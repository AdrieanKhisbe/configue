...
const getTransformForNormalize = (normalize, separator, ignorePrefix) => {
    const normalizer = ignorePrefix ?
        key => _[normalize](_.reduce(ignorePrefix, (acc, prefix) =>
                                                     acc.remove(new RegExp(`^${prefix}`), key))):
        _[normalize];
    if(!separator)
        return ({key, value}) => {
...
const loadArgv = (nconf, options) => {
    if (_.has(options, 'disable.argv')) return;
    ...
    if(options.normalize) yargs.transform = getTransformForNormalize(options.normalize, options.separator);
...
const loadEnv = (nconf, options) => {
    if (_.has(options, 'disable.env')) return;
    ...
    if(options.normalize) envOpts.transform = getTransformForNormalize(options.normalize, options.separator);
