...
const Configue = module.exports = function Configue(options = {}) {
    ...
    this.shortstopResolver.use('file', fs.readFile);
    // TODO LOAD PROTOCOLS
...
Configue.prototype.resolve = function () {
    ...
    if (this.async){
        ...
        Promise.resolve(applyDefaultWorkflowAsync(this.nconf, this.settings)))
            .then(() => resolveShortstops(this.nconf, this.shortstopResolver))
...
const resolveShortstops = (nconf, resolver) => {
    ...
    return Promise.fromCallback(callback => {
        resolver.resolve(originalValues, (err, resolvedValues) => {
            ...
            if (_.get(originalValues, path) !== _.get(resolvedValues, path))
                nconf.set(path.join(':'),  _.get(resolvedValues, path));
            });
            callback(null);
        });
    });
