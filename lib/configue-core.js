...
const load = resource => (nconf, options) => {
    if (!_.has(options, ['disable', resource])) {
...
const loadArgv = (nconf, options) => {
    if (!_.has(options, 'disable.argv')) {
        if (options.argv) {
            if(options.parse !== undefined) _.set(options, 'argv.parse', options.parse);
            if(options.transform) _.set(options, 'argv.transform', options.transform);
        }
        nconf.argv(options.argv);
...
const loadEnv = (nconf, options) => {
    if (!_.has(options, 'disable.env')) {
        ...
        envOpts.parse = options.parse;
        envOpts.transform = options.transform;
        nconf.env(envOpts);
    }
};
