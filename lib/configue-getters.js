...
function getAsync(key, defaultOrCallback) {
    if(typeof defaultOrCallback === 'function') {
        this.nconf.get(key, defaultOrCallback);
    } else {
 ...
 * @param keys the keys to form object with
 * @returns {object} the forged object
 */
function getObject (...keys) {
    const keys = keys.length === 1 && Array.isArray(_.first(keys)) ? _.first(keys): keys;
...
function load(model) {
    return model ?
      // TODO: maybe enrich the `c` function that is given
